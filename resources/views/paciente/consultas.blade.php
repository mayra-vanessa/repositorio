...
@section('content')
    <ul class="breadcrumb">
        ...
        <li><i class="fa fa-heartbeat"></i> Consultas</li>
    </ul>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12 text-center">
                <h1 class="display-5 font-weight-bold">Mis Consultas</h1>
            </div>
        </div>
        <br>
        <div class="row justify-content-center">
            <div class="col-md-6">
                <!-- Cuadro de búsqueda -->
                <form action="{{ route('paciente.consultas') }}" method="GET">
                    <div class="input-group mb-3">
                        ...
                        @if ($search)
                            <a href="{{ route('paciente.consultas') }}" class="btn btn-secondary btn-clear-search">Limpiar búsqueda</a>
                        @endif
                    </div>
                </form>
            </div>
                            ...
                            @foreach ($consultas as $consulta)
                                <tr>
                                    ...
                                    <td style="display: flex; align-items: center;">
                                        <div style="flex: 7;">
                                            {{ $consulta->especialista->nombre_completo }}
                                        </div>
                                        <div style="flex: 3; text-align: right;">
                                            <div class="img-container" style="max-width: 50px;">
                                                <img src="{{ $consulta->especialista->foto }}" class="img-fluid img-especialista-small" alt="{{ $consulta->especialista->nombre_completo }}">
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        @if ($consulta->estatus === 1) <!-- Verificar si la consulta está finalizada -->
                                            <a href="{{ route('receta.ver', ['idconsulta' => $consulta->idconsulta]) }}" class="btn btn-primary">Ver Receta</a>
                                        @endif
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function loadMessages(consultaId) {
                $.get('/Consultas/' + consultaId + '/Comentarios', function (data) {
                    var chatContainer = $('#chatContainer');
                    chatContainer.empty();
                    ...
                    if (data.consulta && data.consulta.comentarios) {
                        data.consulta.comentarios.forEach(function (comentario) {
                            ...
                            chatContainer.append(
                                '<li>' +
                                '<div class="message ' + (comentario.departede === 0 ? 'message-doctor' : 'message-user') + '">' +
                                '<div class="message-content" style="width: 100%; float: left;">' +
                                (comentario.departede === 0 ?
                                    '<img src="{{ $consulta->especialista->foto }}" alt="Doctor" class="user-avatar">' +
                                    '<strong>Dr@. {{ $consulta->especialista->nombre_completo }}</strong>  <br>' : '') +
                                comentario.comentario +
                                (comentario.departede === 0 ?  '<br>' +formatTime(comentario.hora) : '') +
                                (comentario.departede !== 0 ?
            ...
            function sendMessage(message, consultaId) {
                ...
                if (navigator.onLine) {
                    $.ajax({
                        url: '/Consultas/' + consultaId + '/Comentarios/Agregar',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            mensaje: message,
